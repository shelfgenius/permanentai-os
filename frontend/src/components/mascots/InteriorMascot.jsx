...
export default function InteriorMascot({ state = 'idle', audioLevel = 0, size = 220 }) {
  ...
  return (
    <motion.div className="relative select-none" style={{ width: size, height: size }}>
      ...
      {/* Painting / decorating */}
      <motion.div
        style={{ position: 'absolute', bottom: '5%', left: '15%', width: '70%', height: '35%',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: `${Math.round(size * 0.2)}px` }}
        animate={isSpeaking
          ? { scale: [1, 1.1, 1] }
          : { scale: [0.9, 1.05, 0.9], rotateZ: [-2, 2, -2] }}
